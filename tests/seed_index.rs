...
quickcheck! {
    fn seed_to_index_then_back(idx: u32) -> bool {
        ...
        let idx = (idx % 0x0100_0000) as usize;
        let seed = index_to_seed(idx, 4);
        seed_to_index(&seed, 4) == idx
    }
}
...
#[test]
fn edge_cases() {
    let indices: &[usize] = &[0, 0xFF, 0xFFFF, 0xFF_FFFF];
    for &idx in indices {
        let seed = index_to_seed(idx, 4);
        assert_eq!(seed_to_index(&seed, 4), idx);
