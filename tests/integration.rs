...
#[test]
fn cli_gloss_only_roundtrip() {
    let exe = env!("CARGO_BIN_EXE_inchworm");
    let dir = std::env::temp_dir();
    let input = dir.join("inchworm_gloss_input.bin");
    let compressed = dir.join("inchworm_gloss_compressed.bin");
    let output = dir.join("inchworm_gloss_output.bin");
    ...
    assert!(compress.status.success());
    let json: Value = serde_json::from_slice(&compress.stdout).unwrap();
    assert_eq!(json["total_hashes"].as_u64().unwrap(), 0);
