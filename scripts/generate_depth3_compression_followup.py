#!/usr/bin/env python3
"""Run bounded depth-3 compression on rows promoted by the prefix probe."""
...
DOCS = ROOT / "docs"
PREFIX_PROBE_JSON = DOCS / "depth3_prefix_probe.json"
FOLLOWUP_JSON = DOCS / "depth3_compression_followup.json"
FOLLOWUP_MD = DOCS / "DEPTH3_COMPRESSION_FOLLOWUP.md"
...
def artifact_hashes() -> dict[str, str]:
    return {
        "depth3_prefix_probe_sha256": sha256(PREFIX_PROBE_JSON),
    }
...
def promoted_rows(prefix_probe: dict[str, Any]) -> list[dict[str, Any]]:
    rows = [
        ...
        and (
            row.get("prefix_ge_5_delta_vs_depth2", 0) > 0
...
def physical_inputs(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return sorted(grouped.values(), key=lambda item: item["input_sha256"])
...
def alias_summary(row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "transform": row["transform"],
        "prefix_probe": {
            "baseline_prefix_ge_3": row["baseline_prefix_ge_3"],
            "depth3_prefix_ge_3": row["depth3_prefix_ge_3"],
            ...
            "baseline_prefix_ge_6": row["baseline_prefix_ge_6"],
            "depth3_prefix_ge_6": row["depth3_prefix_ge_6"],
...
def input_result(input_group: dict[str, Any], temp: Path, exe: Path) -> dict[str, Any]:
    ...
    return {
        "input_sha256": input_group["input_sha256"],
...
def summarize(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    conclusion = (
        "Bounded depth-3 compression promoted at least one prefix-frontier row into selected spans or negative delta."
        if promoted_rows_count
        else "Bounded depth-3 compression did not turn the current prefix-frontier movement into selected spans or negative delta."
    )
    return {
...
def build_report() -> dict[str, Any]:
    ...
    return {
        "generated_by": "scripts/generate_depth3_compression_followup.py",
...
def check_report() -> None:
    if not FOLLOWUP_JSON.exists() or not FOLLOWUP_MD.exists():
        raise SystemExit("generated depth-3 compression follow-up files are missing")
    payload = load_json(FOLLOWUP_JSON)
    if payload.get("generated_by") != "scripts/generate_depth3_compression_followup.py":
        raise SystemExit("depth3_compression_followup.json has wrong generated_by marker")
    if payload.get("artifact_hashes") != artifact_hashes():
        raise SystemExit("depth3_compression_followup.json artifact hashes are stale")
    if payload.get("selected_manifest_sha256") != selected_manifest_hash():
        raise SystemExit("depth3_compression_followup.json selected manifest hash is stale")
    if len(payload.get("logical_rows", [])) != len(selected_manifest()):
        raise SystemExit("depth3_compression_followup.json logical row count is stale")
    if len(payload.get("results", [])) != len(physical_inputs(promoted_rows(load_json(PREFIX_PROBE_JSON)))):
        raise SystemExit("depth3_compression_followup.json result count is stale")
    text = FOLLOWUP_MD.read_text(encoding="utf-8")
    for phrase in (
        "Depth-3 Compression Follow-Up",
        "prefix-frontier movement",
        "Promotion requires depth-3 selected spans or negative delta",
        "deduplicated by input SHA-256",
        "broad depth-3 sweeps remain gated",
    ):
        if phrase not in text:
            raise SystemExit(f"DEPTH3_COMPRESSION_FOLLOWUP.md missing phrase: {phrase}")
